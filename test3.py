...
# -----------------------------
con = duckdb.connect()
infile1 = get_hive_parquet('CIS_EMPLOYEE_RESIGM_NOTFOUND')
infile2 = get_hive_parquet('CIS_EMPLOYEE_RESIGM')
...
SELECT
    ...
    NULL AS ACCTCODE,
    NULL AS ACCTNOC
FROM read_parquet('{host_parquet_path("CUSTCODE_EMPLM_ERR.parquet")}')
""")
...
# -----------------------------
report_path = csv_output_path(f"CIS_EMPLOYEE_RESIGM_RPT_{report_date_str}").replace(".csv", ".txt")
...
with open(report_path, "w") as rpt:
    ...
    def new_page_header():
        ...
        rpt.write(f"BRANCH      : 0000000{'':25}EXCEPTION REPORT FOR RESIGNED STAFF(MONTHLY)\n")
        rpt.write(" "*46 + "============================================\n")
