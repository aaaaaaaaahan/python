...
duckdb.sql(f"""
       COPY ({final_query})
       TO '{parquet_output_path("CISREPT_UPDATE_REVIEW.parquet")}'
       (FORMAT PARQUET)
""")

duckdb.sql(f"""
       COPY ({final_query})
       TO '{csv_output_path("CISREPT_UPDATE_REVIEW.csv")}'
       (HEADER TRUE, DELIMITER ',')
""")

print("✅ Process complete — REVDATA and UPDREPT successfully generated.")
