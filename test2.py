...
# ======================================================
# Batch date (yesterday) and partition fields
# ======================================================
...
con.execute("""
    ...
    SELECT 
        TRIM(COALESCE(classify, ''))     AS CLASSIFY,
        TRIM(COALESCE(nature, ''))       AS NATURE,
        TRIM(COALESCE(key_code, ''))     AS KEY_CODE,
        TRIM(COALESCE(classcode, ''))    AS CLASSCODE
    FROM RHOB_RAW
""")

con.execute("CREATE TABLE RHOB_UQ AS SELECT DISTINCT * FROM RHOB")
...
con.execute("""
    ...
    SELECT
        TRIM(COALESCE(key_id, ''))        AS KEY_ID,
        TRIM(COALESCE(key_code, ''))      AS KEY_CODE,
        TRIM(COALESCE(key_describe, ''))  AS KEY_DESCRIBE,
        TRIM(COALESCE(key_remark_id1, '')) AS KEY_REMARK_ID1,
        TRIM(COALESCE(key_remark_1, ''))  AS KEY_REMARK_1,
        TRIM(COALESCE(key_remark_id2, '')) AS KEY_REMARK_ID2,
        TRIM(COALESCE(key_remark_2, ''))  AS KEY_REMARK_2,
        TRIM(COALESCE(key_remark_id3, '')) AS KEY_REMARK_ID3,
        TRIM(COALESCE(key_remark_3, ''))  AS KEY_REMARK_3,
        TRIM(COALESCE(desc_lastoperator, '')) AS DESC_LASTOPERATOR,
        TRIM(COALESCE(desc_lastmnt_date, ''))  AS DESC_LASTMNT_DATE,
        TRIM(COALESCE(desc_lastmnt_time, ''))  AS DESC_LASTMNT_TIME
    FROM RHOD_RAW
""")
...
# Deduplicate RHOD by KEY_CODE
con.execute("CREATE TABLE RHOD_UQ AS SELECT DISTINCT * FROM RHOD_EXPANDED")
...
con.execute("""
    ...
    SELECT
        TRIM(COALESCE(key_id1, key_id, '')) AS KEY_ID1,
        TRIM(COALESCE(classify, '')) AS CLASSIFY,
        TRIM(COALESCE(key_describe1, key_describe, '')) AS KEY_DESCRIBE1
    FROM RHOD_RAW
    WHERE TRIM(COALESCE(key_id1, key_id, '')) = 'CLASS'
       OR TRIM(COALESCE(key_id1, key_id, '')) = 'CLASS '
""")

# If RHOD1 is empty (parquet layout different), attempt to create RHOD1 from RHOB's CLASSIFY->KEY_DESCRIBE mapping
res = con.execute("SELECT COUNT(*) FROM RHOD1").fetchone()[0]
if res == 0:
    con.execute("""
        CREATE OR REPLACE TABLE RHOD1 AS
        SELECT
            CLASSIFY,
            '' AS KEY_ID1,
            '' AS KEY_DESCRIBE1
        FROM RHOB_UQ
        WHERE CLASSIFY IS NOT NULL AND CLASSIFY != ''
    """)
    # keep distinct
    con.execute("CREATE OR REPLACE TABLE RHOD1 AS SELECT DISTINCT CLASSIFY, KEY_ID1, KEY_DESCRIBE1 FROM RHOD1")
...
con.execute("""
    ...
    SELECT
        TRIM(COALESCE(classcode, '')) AS CLASSCODE,
        TRIM(COALESCE(indorg, ''))    AS INDORG,
        TRIM(COALESCE(name, ''))      AS NAME,
        TRIM(COALESCE(newic, ''))     AS NEWIC,
        TRIM(COALESCE(othid, ''))     AS OTHID,
        TRIM(COALESCE(crtdtyyyy, '')) AS CRTDTYYYY,
        TRIM(COALESCE(crtdtmm, ''))   AS CRTDTMM,
        TRIM(COALESCE(crtdtdd, ''))   AS CRTDTDD,
        TRIM(COALESCE(dobdtyyyy, '')) AS DOBDTYYYY,
        TRIM(COALESCE(dobdtmm, ''))   AS DOBDTMM,
        TRIM(COALESCE(dobdtdd, ''))   AS DOBDTDD
    FROM RHOLD_RAW
""")
...
# Compute REPTDATE (today - 7) formatted as YYYYMMDD string
rept_date = (datetime.date.today() - datetime.timedelta(days=7)).strftime("%Y%m%d")
con.execute(f"CREATE OR REPLACE TABLE RHOLD_FILTERED AS SELECT * FROM RHOLD WHERE CRTDATE >= '{rept_date}'")

# Deduplicate and sort by CLASSCODE as per SAS PROC SORT
con.execute("CREATE TABLE RHOLD_UQ AS SELECT DISTINCT * FROM RHOLD_FILTERED")
...
# Build query that appends partition columns
out = f"""
    SELECT 
...
# ======================================================
output_name = "HCM_RHOLD_MATCH"

parquet_path = parquet_output_path(output_name)
csv_path = csv_output_path(output_name)

# Parquet partitioned by year/month/day, overwrite if exist
con.execute(f"""
COPY ({out})
TO '{parquet_path}'
...
# CSV (single file per target location); overwrite if exist
con.execute(f"""
COPY ({out})
TO '{csv_path}'
