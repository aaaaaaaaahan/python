...
con.execute(f"""
    ...
    SELECT 
        regexp_replace(ALIASKEY, ';', '', 'g') AS ALIASKEY,
...
con.execute(f"""
    ...
    SELECT DISTINCT ON (ACCTBRCH)
        CAST(BRANCHNO AS VARCHAR)     AS ACCTBRCH,
        CAST(ACCTBRABBR AS VARCHAR)  AS BRANCH_ABBR
...
con.execute(f"""
    ...
    SELECT 
        CAST(DEMOCATEGORY AS VARCHAR)      AS TYPE,
        CAST(DEMOCODE AS VARCHAR)  AS DEMOCODE,
        CAST(RLENDESC AS VARCHAR)  AS DEMODESC
...
con.execute(f"""
    ...
    SELECT 
        CAST(DEMOCODE AS VARCHAR) AS MASCO2008,
        CAST(DESCRIPTION AS VARCHAR) AS MASCODESC
...
con.execute(f"""
    ...
    SELECT 
        CAST(DEMOCODE AS VARCHAR) AS MSICCODE,
        CAST(DESCRIPTION AS VARCHAR) AS MSICDESC
...
# ================================================================
con.execute(f"""
    CREATE VIEW cisfile AS
    SELECT 
        *,
        occup AS democode
    FROM read_parquet('/host/cis/parquet/CIS_CUST_DAILY/year=2025/month=9/day=17/data_0.parquet')
    ORDER BY ALIASKEY, ALIAS
""")
...
con.execute(f"""
    ...
    SELECT
        ...
        CAST(CUSTNAME AS VARCHAR)      AS CUSTNAME_SDB,
        CAST(ALIASKEY AS VARCHAR)      AS ALIASKEY_SDB,
        CAST(ALIAS AS VARCHAR)         AS ALIAS,
        CAST(ACCTBRABBR AS VARCHAR)    AS BRANCH_ABBR,
        CAST(BRANCHNO AS VARCHAR)      AS BRANCHNO,
        CAST(ACCTCODE AS VARCHAR)      AS ACCTCODE,
        CAST(ACCTNO AS VARCHAR)       AS ACCT_NO,
        ...
        '3'                            AS CATEGORY,
        'SDB  '                        AS APPL_CODE
    FROM '{host_parquet_path("SNGLVIEW_SDBX.parquet")}'
    ORDER BY ALIAS
""")
...
con.execute(f"""
    ...
    SELECT
        LPAD(CAST(CAST(ACCTNO AS BIGINT) AS VARCHAR), 11, '0') AS ACCTNOC,
        LPAD(CAST(CAST(ACCTBRCH1 AS BIGINT) AS VARCHAR), 3, '0') AS ACCTBRCH,
        LPAD(CAST(CAST(PRODTYPE AS BIGINT) AS VARCHAR), 3, '0') AS PRODTY,
...
con.execute(f"""
    ...
    FROM '{host_parquet_path("SNGLVIEW_PBCS.parquet")}'
    ORDER BY ACCTNOC
""")
...
con.execute(f"""
    ...
    SELECT
        ...
        CAST(DATEOPEN AS VARCHAR)     AS DATEOPEN,
        CAST(DATECLSE AS VARCHAR)     AS DATECLSE
    FROM '{host_parquet_path("SNGLVIEW_COMCARD.parquet")}'
    ORDER BY ACCTNOC
""")
